...
//Negative case 2 
test('Negative case 2 - gagal Login', async ({ page }) => {
    const username_field = page.locator("//input[@id='userName']")
    const password_field = page.locator("//input[@id='password']")
    const button = page.locator("//button[@type='submit']")
...
await page.goto('https://panin-ccpl-site.skyworx.co.id/');
  ...
  await username_field.waitFor();
  await username_field.fill(loginSalah.username)

  await password_field.waitFor();
  await password_field.fill(loginSalah.password)

  await button.waitFor
  await button.click();

  const dialog = page.locator('//div[contains(@class,"swal2-popup") and @role="dialog"]');
  await expect(dialog).toBeVisible({ timeout: 10000 });

  const title = page.locator('//h2[@id="swal2-title" and normalize-space(text())="Failed"]');
  await expect(title).toBeVisible();
});


//Positive case 3
test('Positive case 3 - Log out', async ({ page }) => {
    const username_field = page.locator("//input[@id='userName']")
    const password_field = page.locator("//input[@id='password']")
    const button = page.locator("//button[@type='submit']")
    const button_profile = page.locator("(//button[@type='button'])[3]")
